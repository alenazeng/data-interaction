<script>
  ...
  $: yScale = d3.scaleBand()
    .range([0, chartH])
    .domain(sortedData.map(d => d.community_name))
    .padding(0.15);
  ...
  function computeAnnShape(ann, sorted, xSc, ySc, xV, cw, ch) {
    ...
    if (ann.direction === "above") {
      ...
      cutY = next
        ? (ySc(last.community_name) + ySc.bandwidth() + ySc(next.community_name)) / 2
        : ch;
      ...
      cutY = prev
        ? (ySc(prev.community_name) + ySc.bandwidth() + ySc(first.community_name)) / 2
        : 0;
  ...
  $: if (barLayer && xAxisEl && sortedData.length && xVar) {
    render(sortedData, xScale, yScale, xVar, highlightedCities, selectedCommunity);
  }

  function render(sorted, xSc, ySc, xV, highlighted, selected) {
    ...
    const groups = d3.select(barLayer)
      ...
      .data(sorted, d => d.community_name);

    // ENTER
    const enter = groups.enter()
      .append("g")
      .attr("class", "bar-group")
      .attr("transform", d => `translate(0, ${ySc(d.community_name)})`);

    enter.append("rect")
      .attr("x", 0)
      .attr("y", 0)
      .attr("height", ySc.bandwidth())
      // .attr("fill", d => highlighted.includes(d.community_name) ? "crimson" : "steelblue")
      .attr("fill", d => {
        if (!selected) {
          return highlighted.includes(d.community_name)
            ? "crimson"
            : "steelblue";
        }

        return d.community_name === selected
          ? "crimson"
          : "#d3d3d3";
      })
      .attr("opacity", d => {
