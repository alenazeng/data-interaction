<script>
  ...
  function computeSlots(layout, W, H) {
    ...
    const defaults = {
      ...
      map:     { x: R, y: PAD, w: half, h: (h - GAP) * 0.6 },
  ...
  onMount(async () => {
    data = await d3.csv("env-income-data.csv", d => ({ 
      ...
      environmentalJusticeImpactRate: +d["CHAVCNN_2023"],
      particulate_matter: +d["PMC_2020"],
      household_income: +d["INC_2020-2024"],
      poverty_rate: +d["POV_2020-2024"],
      college_grad: +d["EDE_2020-2024"]

    }));

    const geo = await d3.json("community-areas.geojson");
    stateFeatures = geo;
    stateMesh = null;
...
<main>
    <div class="scrollyteller">
        <section class="title text">
            <h1>Socioeconomic Outcomes in Chicago's Community Areas</h1>
        </section>
        <div class="text">
            {#each stages as s, i}
            <section
